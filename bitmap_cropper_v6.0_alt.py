...
class BitmapCropperApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Bitmap Font Repacker V6.0")
        ...
        self.combo_scale = ttk.Combobox(r2, values=["1.0x", "2.0x", "3.0x", "4.0x"], state="readonly", width=5)
    ...
    def open_preview_window(self):
        ...
        # 1. Info Panel (Restored from V5)
        self.preview_info_label = ttk.Label(top, text="Initializing...", font=("Arial", 9))
        self.preview_info_label.pack(side="top", fill="x", padx=5, pady=5)
        ...
        # 4. Status Bar (Restored from V5)
        status_frame = ttk.Frame(top, relief="sunken")
        ...
        self.preview_canvas.bind("<Motion>", self.on_preview_mouse_move) # New binding for coords
    ...
    def refresh_preview_if_open(self, event=None):
        ...
        if not data: return
        img, boxes, _, _, cfg = data
        self.preview_boxes = boxes # Store for click detection
        
        # Draw boxes on copy
        prev = img.copy()
        draw = ImageDraw.Draw(prev)
        
        relocs = self.relocation_map.values() # Highlight sources that are used
